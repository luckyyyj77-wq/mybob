...
export default function DiagnosisPage() {
  ...
  useEffect(() => {
    ...
    if (daysSince >= 7) {
      setRefreshBanner(t('refreshDays').replace('{n}', String(daysSince)));
    } else if (newMealsSince >= 20) {
      setRefreshBanner(t('refreshMeals').replace('{n}', String(newMealsSince)));
    }
  }, [meals, current]);
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const previousScores = history.slice(0, 3).map(r => ({
        date: fmtDate(toKSTDate(r.analyzedAt), locale),
      ...
      const label = new Date().toLocaleDateString(locale === 'ko' ? 'ko-KR' : 'en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' });
  ...
  if (plan === 'free') {
    return (
      <div style={{ padding: '40px 24px', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: '15px', color: 'black', marginBottom: '8px' }}>{t('proOnly')}</p>
        <p style={{ fontSize: '12px', color: '#9ca3af', lineHeight: 1.8, marginBottom: '24px', whiteSpace: 'pre-line' }}>
          {t('proOnlyDesc')}
        </p>
        <div style={{ border: '1px solid #e5e7eb', padding: '20px', marginBottom: '20px', textAlign: 'left' }}>
          <p style={{ fontSize: '10px', color: '#9ca3af', letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '12px' }}>{t('proFeatureList')}</p>
          {(t.raw('proFeatureItems') as string[]).map(f => (
    ...
    <div style={{ padding: '20px 24px 60px' }}>
      ...
      {!current && !loading && (
        <div style={{ border: '1px solid #e5e7eb', padding: '24px', marginBottom: '16px', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: '15px', color: 'black', marginBottom: '6px' }}>{t('diagTitle')}</p>
          <p style={{ fontSize: '12px', color: '#9ca3af', lineHeight: 1.7, marginBottom: '20px', whiteSpace: 'pre-line' }}>
            {t('diagDesc')}
          </p>
          <p style={{ fontSize: '11px', color: '#6B21A8', marginBottom: '16px' }}>{t('diagDataCount').replace('{n}', String(recentMeals.length))}</p>
      ...
      {loading && (
        <div style={{ textAlign: 'center', padding: '60px 0' }}>
          ...
          <p style={{ fontSize: '13px', color: '#9ca3af', letterSpacing: '1px' }}>{t('analyzing')}</p>
          <p style={{ fontSize: '11px', color: '#d1d5db', marginTop: '6px' }}>{t('analyzingTime')}</p>
        </div>
      )}
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          ...
          <div style={{ backgroundColor: '#f9fafb', border: '1px solid #e5e7eb', padding: '10px 14px', marginBottom: '16px', display: 'flex', flexWrap: 'wrap', gap: '6px 16px' }}>
            <span style={{ fontSize: '10px', color: '#9ca3af' }}>
              📅 {t('analysisPeriod')}: <span style={{ color: '#374151' }}>{fmtDate(current.stats.periodStart, locale)} ~ {fmtDate(current.stats.periodEnd, locale)} ({t('daysUnit').replace('{n}', String(current.stats.days))})</span>
            </span>
            <span style={{ fontSize: '10px', color: '#9ca3af' }}>
              🍽 {t('mealBase')}: <span style={{ color: '#374151' }}>{current.stats.meals}</span>
            </span>
            <span style={{ fontSize: '10px', color: '#9ca3af' }}>
              🕐 {t('analyzedAt')}: <span style={{ color: '#374151' }}>{current.analyzedAtLabel}</span>
            </span>
          </div>
          ...
          <div style={{ border: '1px solid #e5e7eb', padding: '20px', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '20px' }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
                ...
                <span style={{ fontSize: '10px', color: '#9ca3af', letterSpacing: '1px' }}>{t('gradeLabel')}</span>
                {/* 이전 진단 대비 변화 */}
                {history.length > 1 && (() => {
          ...
          <div style={{ display: 'flex', borderBottom: '1px solid #e5e7eb', marginBottom: '16px' }}>
            {(['overview', 'detail', 'plan', 'trend'] as const).map((key) => (
              <button key={key} onPointerDown={() => setActiveTab(key)}
                ...
                {key === 'overview' ? t('tabOverview') : key === 'detail' ? t('tabDetail') : key === 'plan' ? t('tabPlan') : t('tabTrend')}
              </button>
            ))}
          </div>
          ...
          <AnimatePresence mode="wait">
            ...
            {activeTab === 'overview' && (
              <motion.div key="overview" initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                <div style={{ marginBottom: '16px' }}>
                  <p style={{ fontSize: '9px', color: '#9ca3af', letterSpacing: '1.5px', textTransform: 'uppercase', marginBottom: '10px' }}>{t('indicatorScores')}</p>
                ...
                {current.diagnosis.strengths.length > 0 && (
                  <div style={{ border: '1px solid #dcfce7', backgroundColor: '#f0fdf4', padding: '14px', marginBottom: '12px' }}>
                    <p style={{ fontSize: '9px', color: '#16a34a', letterSpacing: '1.5px', textTransform: 'uppercase', marginBottom: '8px' }}>{t('strengths')}</p>
                ...
                <div style={{ border: '1px solid #e9d5ff', backgroundColor: '#faf5ff', padding: '16px', marginBottom: '16px' }}>
                  <p style={{ fontSize: '9px', color: '#6B21A8', letterSpacing: '1.5px', textTransform: 'uppercase', marginBottom: '8px' }}>{t('aiCoach')}</p>
                ...
                >
                  {analyzedToday ? t('analyzedToday') : t('reanalyze')}
                </button>
            ...
            {activeTab === 'detail' && (
              <motion.div key="detail" initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '16px' }}>
                  {[
                    { label: t('avgCal'), value: `${current.stats.avgCal} kcal` },
                    { label: t('targetCal'), value: `${current.stats.targetCalories} kcal` },
                    { label: t('analysisDays'), value: t('daysUnit').replace('{n}', String(current.stats.days)) },
                    { label: t('totalLogs'), value: String(current.stats.meals) },
                  ].map(item => (
                ...
                {current.diagnosis.issues.length > 0 && (
                  <div style={{ marginBottom: '16px' }}>
                    <p style={{ fontSize: '9px', color: '#9ca3af', letterSpacing: '1.5px', textTransform: 'uppercase', marginBottom: '10px' }}>{t('issues')}</p>
                ...
                <div>
                  <p style={{ fontSize: '9px', color: '#9ca3af', letterSpacing: '1.5px', textTransform: 'uppercase', marginBottom: '10px' }}>{t('recommendations')}</p>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            {activeTab === 'plan' && (
              <motion.div key="plan" initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                <p style={{ fontSize: '9px', color: '#9ca3af', letterSpacing: '1.5px', textTransform: 'uppercase', marginBottom: '12px' }}>{t('weeklyPlan')}</p>
              ...
              <motion.div key="trend" initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                {history.length < 2 ? (
                  <div style={{ textAlign: 'center', padding: '40px 0' }}>
                    <p style={{ fontSize: '13px', color: '#9ca3af', lineHeight: 1.8, whiteSpace: 'pre-line' }}>
                      {t('trendEmpty')}
                    </p>
                  </div>
                ) : (
                  <>
                    <p style={{ fontSize: '9px', color: '#9ca3af', letterSpacing: '1.5px', textTransform: 'uppercase', marginBottom: '12px' }}>{t('trendTitle')}</p>
                      ...
                      {history.map((r, i) => {
                        ...
                        return (
                          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px', border: i === 0 ? '1px solid #d8b4fe' : '1px solid #e5e7eb', backgroundColor: i === 0 ? '#faf5ff' : 'white' }}>
                            ...
                            <div style={{ flex: 1 }}>
                              <p style={{ fontSize: '10px', color: '#9ca3af', marginBottom: '2px' }}>{fmtDate(toKSTDate(r.analyzedAt), locale)} · {t('daysUnit').replace('{n}', String(r.stats.days))} · {r.stats.meals}</p>
                    ...
                    {/* 지표별 최신 vs 이전 비교 */}
                    <p style={{ fontSize: '9px', color: '#9ca3af', letterSpacing: '1.5px', textTransform: 'uppercase', marginBottom: '10px' }}>{t('indicatorTrend')}</p>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      {Object.entries(current.diagnosis.scores).map(([key, val]) => {
