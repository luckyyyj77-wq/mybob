    ...
    if (mode === 'verify') {
      ...
      if (next.length === 4) {
        if (verifyPin(next)) {
          ...
          if (context === 'body') {
            ...
            if (left <= 0) { setError(t('locked')); setTimeout(() => { localStorage.removeItem(BODY_ENC_KEY); localStorage.removeItem(BODY_SALT_KEY); resetBodyAttempts(); onCancel(); }, 1500); }
            else if (count >= BODY_WARN_AT) { setError(`${t('wrong')} · ${t('lockedWarn', { count: left })}`); setTimeout(() => setPin(''), 600); }
            else { setError(t('wrong')); setTimeout(() => setPin(''), 600); }
          } else { setError(t('wrong')); setTimeout(() => setPin(''), 600); }
        }
      }
    } else {
      if (step === 'enter') { const next = (pin + d).slice(0, 4); setPin(next); if (next.length === 4) setStep('confirm'); }
        ...
        if (next.length === 4) {
          ...
          else { setError(t('mismatch')); setTimeout(() => { setConfirmPin(''); setStep('enter'); setPin(''); setError(''); }, 800); }
        }
      }
    }
  };
    ...
    <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ backgroundColor: 'white', width: '280px', padding: '32px 24px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '24px' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ fontSize: '15px', color: 'black', marginBottom: '4px' }}>{mode === 'verify' ? t('enter') : step === 'enter' ? t('set') : t('confirm')}</p>
          <p style={{ fontSize: '11px', color: '#9ca3af' }}>{mode === 'verify' ? t('enterDesc') : step === 'enter' ? t('setDesc') : t('confirmDesc')}</p>
        </div>
        ...
        {warned && !error && <p style={{ fontSize: '11px', color: '#ef4444', marginTop: '-12px', textAlign: 'center', lineHeight: 1.5 }}>{t('lockedWarn', { count: remaining })}</p>}
        {error && <p style={{ fontSize: '11px', color: '#ef4444', marginTop: '-12px', textAlign: 'center', lineHeight: 1.5 }}>{error}</p>}
        <div style={{ gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px', width: '100%', display: 'grid' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}>
          <button onClick={onCancel} style={{ fontSize: '12px', color: '#9ca3af', background: 'none', border: 'none', cursor: 'pointer', padding: '4px' }}>{tc('cancel')}</button>
          {mode === 'verify' && context === 'danger' && onForgot && (
            <button onClick={onForgot} style={{ fontSize: '11px', color: '#6B21A8', background: 'none', border: 'none', cursor: 'pointer', padding: '4px' }}>{t('forgot')}</button>
          )}
        </div>
      </div>
    </div>
...
function DangerPinResetModal({ onSuccess, onCancel }: { onSuccess: () => void; onCancel: () => void }) {
  ...
  const handleSend = async () => {
    ...
    if (!authToken) { setError(tc('error')); setSending(false); return; }
  ...
  const handleVerify = async () => {
    ...
    if (!authToken) { setError(tc('error')); setVerifying(false); return; }
    ...
    else { const msg: Record<string, string> = { WRONG_OTP: t('wrong'), OTP_EXPIRED: t('expired') }; setError(msg[result.error] || t('fail')); }
    setVerifying(false);
  };
    ...
    <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ backgroundColor: 'white', width: '300px', padding: '28px 24px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ fontSize: '15px', color: 'black', marginBottom: '4px' }}>{step === 'send' ? t('title') : t('titleVerify')}</p>
          <p style={{ fontSize: '11px', color: '#9ca3af', lineHeight: 1.5 }}>{step === 'send' ? t('desc') : t('descVerify', { email })}</p>
        </div>
        {step === 'verify' && (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {step === 'send' ? (
            <button onClick={handleSend} disabled={sending} style={{ padding: '12px', border: 'none', backgroundColor: sending ? '#f3f4f6' : 'black', color: sending ? '#9ca3af' : 'white', fontSize: '13px', cursor: sending ? 'not-allowed' : 'pointer' }}>{sending ? t('sending') : t('send')}</button>
          ) : (
            <>
              <button onClick={handleVerify} disabled={verifying || otp.length !== 6} style={{ padding: '12px', border: 'none', backgroundColor: (verifying || otp.length !== 6) ? '#f3f4f6' : 'black', color: (verifying || otp.length !== 6) ? '#9ca3af' : 'white', fontSize: '13px', cursor: (verifying || otp.length !== 6) ? 'not-allowed' : 'pointer' }}>{verifying ? t('verifying') : t('verify')}</button>
              <button onClick={() => { setStep('send'); setOtp(''); setError(''); }} style={{ padding: '8px', border: '1px solid #e5e7eb', backgroundColor: 'white', fontSize: '12px', color: '#6b7280', cursor: 'pointer' }}>{t('resend')}</button>
            </>
          )}
          <button onClick={onCancel} style={{ padding: '8px', background: 'none', border: 'none', fontSize: '12px', color: '#9ca3af', cursor: 'pointer' }}>{tc('cancel')}</button>
        </div>
      </div>
    </div>
  );
...
export default function AccountPage() {
  ...
  const handleNicknameSave = async () => {
    ...
    if (nick.length < 2 || nick.length > 16) return alert(t('nickRule'));
    ...
    else if (result.error === 'NICKNAME_ALREADY_CHANGED') alert(t('nickOnce'));
    else alert(result.error || tc('error'));
  ...
  return (
    <div style={{ height: 'calc(100svh - 65px)', display: 'flex', flexDirection: 'column', backgroundColor: 'white', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '24px' }}>
        ...
        {/* 프로필 */}
        <p style={{ fontSize: '10px', color: '#9ca3af', letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '12px' }}>{t('profile')}</p>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                    ...
                    <div>
                      <p style={{ fontSize: '15px', color: 'black', marginBottom: '3px' }}>{profile.nickname || t('noNickname')}</p>
                      <p style={{ fontSize: '11px', color: planStatus?.plan === 'free' ? '#9ca3af' : '#6B21A8', letterSpacing: '0.5px' }}>
                        {tp(planStatus?.plan ?? 'free').toUpperCase()}
                      </p>
                    </div>
                  </div>
                  {planStatus?.plan !== 'free' && planStatus && (
                    <button onClick={() => setShowEditPopup(true)} style={{ fontSize: '12px', color: '#6B21A8', background: 'none', border: '1px solid #e9d5ff', padding: '6px 12px', cursor: 'pointer', letterSpacing: '0.5px' }}>{tc('edit')}</button>
                  )}
                </div>
        ...
        {/* 개인정보 */}
        <p style={{ fontSize: '10px', color: '#9ca3af', letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '12px' }}>{t('privacy')}</p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1px', backgroundColor: '#e5e7eb', marginBottom: '28px' }}>
          <button onClick={() => setShowPrivacyModal(true)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', backgroundColor: 'white', border: 'none', cursor: 'pointer', width: '100%', textAlign: 'left' }}>
            <span style={{ fontSize: '14px', color: 'black' }}>{t('privacyPolicy')}</span>
          ...
          <Link href="/auth/reset-password" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', backgroundColor: 'white' }}>
            <span style={{ fontSize: '14px', color: 'black' }}>{t('resetPassword')}</span>
            <span style={{ fontSize: '16px', color: '#9ca3af' }}>›</span>
          </Link>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1px', backgroundColor: '#e5e7eb', marginBottom: '28px' }}>
          <button onClick={() => setLogoutModal(true)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', backgroundColor: 'white', border: 'none', cursor: 'pointer', width: '100%', textAlign: 'left' }}>
            <span style={{ fontSize: '14px', color: 'black' }}>{t('logout')}</span>
        ...
        {logoutModal && (
          <div onClick={() => setLogoutModal(false)} style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
            <div onClick={e => e.stopPropagation()} style={{ backgroundColor: 'white', width: '100%', maxWidth: '360px', padding: '28px 24px 24px' }}>
              ...
              <h3 style={{ fontSize: '18px', fontWeight: 500, color: 'black', marginBottom: '8px' }}>{t('logoutTitle')}</h3>
              <p style={{ fontSize: '13px', color: '#6b7280', marginBottom: '24px', lineHeight: 1.6 }}>{t('logoutDesc')}</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1px', backgroundColor: '#e5e7eb', marginBottom: '16px' }}>
                <button onClick={() => handleLogout(false)} style={{ padding: '16px', backgroundColor: 'white', border: 'none', cursor: 'pointer', textAlign: 'left' }}>
                  <p style={{ fontSize: '14px', color: 'black', marginBottom: '3px' }}>{t('logoutKeep')}</p>
                  <p style={{ fontSize: '11px', color: '#9ca3af' }}>{t('logoutKeepDesc')}</p>
                </button>
                <button onClick={() => handleLogout(true)} style={{ padding: '16px', backgroundColor: 'white', border: 'none', cursor: 'pointer', textAlign: 'left' }}>
                  <p style={{ fontSize: '14px', color: '#ef4444', marginBottom: '3px' }}>{t('logoutDelete')}</p>
                  <p style={{ fontSize: '11px', color: '#9ca3af' }}>{t('logoutDeleteDesc')}</p>
                </button>
              </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
          <p style={{ fontSize: '10px', color: '#ef4444', letterSpacing: '2px', textTransform: 'uppercase' }}>{t('dangerZone')}</p>
          <button onClick={handleDangerUnlock} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '5px 10px', border: `1px solid ${dangerUnlocked ? '#ef4444' : '#e5e7eb'}`, backgroundColor: dangerUnlocked ? '#fef2f2' : 'white', cursor: 'pointer', fontSize: '11px', color: dangerUnlocked ? '#ef4444' : '#9ca3af', transition: 'all 0.2s' }}>
            ...
            {dangerUnlocked ? t('dangerUnlock') : t('dangerLock')}
          </button>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1px', backgroundColor: '#e5e7eb', marginBottom: '40px', opacity: dangerUnlocked ? 1 : 0.4, transition: 'opacity 0.2s', position: 'relative' }}>
          ...
          <div style={{ padding: '14px 16px', backgroundColor: 'white' }}>
            <p style={{ fontSize: '10px', color: '#9ca3af', letterSpacing: '1.5px', textTransform: 'uppercase', marginBottom: '6px' }}>{t('emailLabel')}</p>
            <p style={{ fontSize: '14px', color: 'black' }}>{userEmail || t('loginRequired')}</p>
          </div>
          ...
          <div style={{ padding: '14px 16px', backgroundColor: 'white' }}>
            <p style={{ fontSize: '11px', color: '#9ca3af', marginBottom: '10px', lineHeight: 1.5 }}>{t('deleteLocalDesc')}</p>
            <div style={{ display: 'flex', gap: '8px' }}>
              <button onClick={handleDeleteAll} disabled={!dangerUnlocked} style={{ padding: '8px 14px', backgroundColor: confirmDelete ? '#ef4444' : 'white', color: confirmDelete ? 'white' : '#ef4444', border: '1px solid #fca5a5', fontSize: '13px', cursor: dangerUnlocked ? 'pointer' : 'not-allowed', transition: 'all 0.2s' }}>
                {confirmDelete ? t('deleteLocalConfirm') : t('deleteLocal')}
              </button>
              {confirmDelete && <button onClick={() => setConfirmDelete(false)} style={{ padding: '8px 14px', backgroundColor: 'white', color: '#9ca3af', border: '1px solid #e5e7eb', fontSize: '13px', cursor: 'pointer' }}>{tc('cancel')}</button>}
            </div>
          </div>

          <div style={{ padding: '14px 16px', backgroundColor: 'white' }}>
            <p style={{ fontSize: '11px', color: '#9ca3af', marginBottom: '10px', lineHeight: 1.5 }}>{t('withdrawDesc')}</p>
            <button
          ...
          {withdrawModal && (
            <div onClick={() => !withdrawing && setWithdrawModal(false)} style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
              <div onClick={e => e.stopPropagation()} style={{ backgroundColor: 'white', width: '100%', maxWidth: '360px', padding: '28px 24px 24px' }}>
                ...
                <h3 style={{ fontSize: '18px', fontWeight: 500, color: 'black', marginBottom: '16px' }}>{t('withdrawConfirm')}</h3>

                <div style={{ backgroundColor: '#fef2f2', border: '1px solid #fca5a5', padding: '14px 16px', marginBottom: '24px' }}>
                  <p style={{ fontSize: '13px', color: '#ef4444', fontWeight: 500, marginBottom: '8px' }}>{t('withdrawWarn')}</p>
                  <ul style={{ fontSize: '12px', color: '#6b7280', lineHeight: 2, paddingLeft: '16px', margin: 0 }}>
                    {(t.raw('withdrawList') as string[]).map((item, i) => <li key={i}>{item}</li>)}
                  </ul>
                  <p style={{ fontSize: '12px', color: '#ef4444', marginTop: '10px', fontWeight: 500 }}>{t('withdrawNoRecover')}</p>
                </div>
        ...
        <div onClick={() => setShowEditPopup(false)} style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 9000, display: 'flex', alignItems: 'flex-end' }}>
          <div onClick={e => e.stopPropagation()} style={{ backgroundColor: 'white', width: '100%', borderRadius: '16px 16px 0 0', padding: '24px 24px 40px', maxHeight: '80vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
              <p style={{ fontSize: '15px', color: 'black' }}>{t('profile')}</p>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '28px' }}>
              ...
              <button onClick={() => !avatarChangedToday && avatarInputRef.current?.click()} style={{ fontSize: '13px', background: 'none', padding: '8px 16px', cursor: avatarChangedToday ? 'default' : 'pointer', border: '1px solid #e9d5ff', color: avatarChangedToday ? '#d1d5db' : '#6B21A8' }}>
                {avatarChangedToday ? t('photoChanged') : t('changePhoto')}
              </button>
            </div>
            <p style={{ fontSize: '10px', color: '#9ca3af', letterSpacing: '1.5px', textTransform: 'uppercase', marginBottom: '8px' }}>{t('nickname')}</p>
            {profile.nickname_changed ? (
              ...
              <div style={{ marginBottom: '24px' }}>
                <div style={{ display: 'flex', gap: '8px', marginBottom: '6px' }}>
                  <input type="text" value={nicknameInput} onChange={e => setNicknameInput(e.target.value)} maxLength={16} placeholder={t('nickname')}
                  ...
                  <button onClick={handleNicknameSave} disabled={nicknameSaving || nicknameInput.trim() === (profile.nickname ?? '') || nicknameInput.trim().length < 2}
                    ...
                    {nicknameSaved ? t('saved') : t('save')}
                  </button>
                </div>
                <p style={{ fontSize: '11px', color: '#ef4444' }}>{t('nickWarning')}</p>
              </div>
            )}
            <p style={{ fontSize: '10px', color: '#9ca3af', letterSpacing: '1.5px', textTransform: 'uppercase', marginBottom: '8px' }}>{t('statusMsg')}</p>
            <div style={{ display: 'flex', gap: '8px', marginBottom: '10px' }}>
              <input type="text" value={statusInput} onChange={e => setNicknameInput(e.target.value)} maxLength={40} placeholder={t('statusMsg')} disabled={statusChangedToday}
              ...
              <button onClick={handleStatusSave} disabled={statusChangedToday || statusSaving || statusInput.trim() === statusMsg}
                ...
                {statusSaved ? t('saved') : statusChangedToday ? tc('confirm') : t('save')}
              </button>
            </div>
            <div style={{ paddingBottom: '32px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
                <p style={{ fontSize: '10px', color: '#9ca3af', letterSpacing: '1.5px', textTransform: 'uppercase' }}>{t('suggestedMsg')}</p>
                {!statusChangedToday && <button onClick={() => setSuggestedMsgs(pickRandom3(statusInput))} style={{ fontSize: '11px', color: '#6B21A8', background: 'none', border: 'none', cursor: 'pointer', padding: '2px 0' }}>{t('otherMsg')}</button>}
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {suggestedMsgs.map(t_msg => (
                  <button key={t_msg.id} onClick={() => !statusChangedToday && setStatusInput(t_msg.text)}
                    style={{ padding: '11px 12px', border: `1px solid ${statusInput === t_msg.text ? '#6B21A8' : '#e5e7eb'}`, backgroundColor: statusInput === t_msg.text ? '#f3e8ff' : 'white', color: statusChangedToday ? '#9ca3af' : statusInput === t_msg.text ? '#6B21A8' : '#6b7280', fontSize: '12px', cursor: statusChangedToday ? 'default' : 'pointer', textAlign: 'left', lineHeight: 1.5 }}>
                    {t_msg.text}
                  </button>
                ))}
              </div>
        ...
        <div onClick={() => setShowPrivacyModal(false)} style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 9000, display: 'flex', alignItems: 'flex-end' }}>
          <div onClick={e => e.stopPropagation()} style={{ backgroundColor: 'white', width: '100%', maxHeight: '75vh', overflowY: 'auto', borderRadius: '12px 12px 0 0', padding: '24px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h3 style={{ fontSize: '16px', fontWeight: 400 }}>{t('privacyPolicy')}</h3>
            ...
            {(t.raw('privacyModal.items') as any[]).map((item, i) => (
              <div key={i} style={{ display: 'flex', gap: '10px', marginBottom: '16px' }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: '11px', color: '#6b7280', lineHeight: 1.6 }}>
                    {item.content || (storageMode === 'cloud' ? item.cloud : item.local)}
                  </p>
                </div>
              </div>
            ))}
